...
import org.camunda.bpm.extension.process_test_coverage.plugin.ProcessTestCoverageEnabledEngine;
 ...
 */
public class ProcessTestCoverageRule extends TestWatcher {

  private static Logger logger = Logger.getLogger(ProcessTestCoverageRule.class.getCanonicalName());
  ...
  public ProcessTestCoverageRule() {
    super();
  }
  ...
  private void initializeMethodCoverage(Description description) {
      ...
      final List<ProcessDefinition> deployedProcessDefinitions = processEngine().getRepositoryService().createProcessDefinitionQuery().list();

      if (!deployedProcessDefinitions.isEmpty()) {
        coverageTestRunState.initializeTestMethodCoverage(
            processEngine(),
            deployedProcessDefinitions.get(0).getDeploymentId(),
  ...
  private ProcessEngine processEngine() {
    return ProcessTestCoverageEnabledEngine.getProcessEngine();
  }
